...
const AIPromptGenerator = ({ playerData }) => {
  ...
  return (
    <View style={styles.aiPromptContainer}>
      <Text style={styles.aiPromptTitle}>🤖 AI Assistant</Text>
...
export default function PlayerProfileScreen({ route, navigation }) {
  ...
  }, [sportsData, addToSearchHistory, playerData]);
              ...
              >
                <View style={styles.playerResultInfo}>
                  ...
                  <Text style={styles.playerResultDetails}>
                    {item.team} • {item.position} • {item.stats?.points || item.points || 'N/A'} PPG
                  </Text>
                </View>
      ...
      <View style={styles.sectionContainer}>
        ...
        <View style={styles.quickActionsGrid}>
          <TouchableOpacity 
            ...
            onPress={async () => {
              await logAnalyticsEvent('player_profile_compare', {
                player_name: enhancedPlayerData.name,
              });
            }}
            activeOpacity={0.7}
          ...
          <TouchableOpacity 
            style={styles.quickAction}
            onPress={async () => {
              await logAnalyticsEvent('player_profile_fantasy', {
                player_name: enhancedPlayerData.name,
              });
            }}
            activeOpacity={0.7}
          ...
          <TouchableOpacity 
            style={styles.quickAction}
            onPress={async () => {
              await logAnalyticsEvent('player_profile_highlights', {
                player_name: enhancedPlayerData.name,
              });
            }}
            activeOpacity={0.7}
          ...
          <TouchableOpacity 
            style={styles.quickAction}
            onPress={async () => {
              await logAnalyticsEvent('player_profile_injury', {
                player_name: enhancedPlayerData.name,
              });
            }}
            activeOpacity={0.7}
