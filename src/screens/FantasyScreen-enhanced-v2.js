...
const FantasyScreen = () => {
  ...
  useEffect(() => {
    fantasyData.myTeam.players.forEach(player => {
      progressAnimations.current[player.id] = new AnimatedProgress.Value(player.points / 50);
    });
  }, []);
  ...
  const loadData = async () => {
    try {
      ...
      // Log analytics event for screen refresh
      await analytics().logEvent('fantasy_screen_refresh', {
      ...
      // Log error analytics
      await analytics().logEvent('fantasy_data_error', {
        error: error.message,
        league: selectedLeague,
      });
    } finally {
  ...
  useEffect(() => {
    if (hasAccess) {
      ...
      analytics().logEvent('fantasy_screen_view', {
        screen_name: 'FantasyScreen',
  ...
  const handleLeagueSelect = async (league) => {
    ...
    await analytics().logEvent('fantasy_league_select', {
      league,
      previous_league: selectedLeague,
    });
  };

  const handleProjectionsToggle = async (value) => {
    setShowProjections(value);
    
    // Log analytics event for toggle
    await analytics().logEvent('fantasy_projections_toggle', {
      enabled: value,
      league: selectedLeague,
    });
  };
      ...
      <View style={styles.accessDenied}>
        ...
        <TouchableOpacity 
          ...
          onPress={async () => {
            ...
            // Log analytics event for upgrade attempt
            await analytics().logEvent('fantasy_upgrade_attempt', {
        ...
        <TouchableOpacity 
          ...
          onPress={async () => {
            ...
            if (result.success && result.restoredEntitlements.includes('premium_access')) {
              ...
              await analytics().logEvent('purchase_restore_success', {
              ...
              // Log failed restore
              await analytics().logEvent('purchase_restore_failed', {
      ...
      <View style={styles.sectionHeader}>
        ...
        <TouchableOpacity style={styles.controls}>
          ...
          <Switch
            ...
            onValueChange={handleProjectionsToggle}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={async () => {
              await analytics().logEvent('fantasy_manage_roster', {
                league: selectedLeague,
              });
            }}>
