...
// Correct Firebase Analytics import and usage
...
export default function ParlayBuilderScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      await logFirebaseEvent('parlay_builder_load', {
        has_nba_data: !!nba,
        has_nfl_data: !!nfl,
        has_nhl_data: !!nhl,
      });
      ...
      // Log error analytics
      await logFirebaseEvent('parlay_builder_error', {
        error: error.message,
      });
  ...
  const calculateParlay = useCallback(async (picks) => {
    ...
    await logFirebaseEvent('parlay_calculated', {
  ...
  const addPlayerPick = useCallback(async (player) => {
    ...
    setSelectedPicks(prev => {
      ...
      logFirebaseEvent('parlay_pick_added', {
        pick_type: 'player',
  ...
  const removePick = useCallback(async (id, pick) => {
    ...
    await logFirebaseEvent('parlay_pick_removed', {
  ...
  const runMonteCarloSimulation = useCallback(async () => {
    ...
    await logFirebaseEvent('parlay_simulation_run', {
  ...
  const analyzeParlay = useCallback(async () => {
    ...
    await logFirebaseEvent('parlay_analysis_viewed', {
      num_picks: selectedPicks.length,
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      await logFirebaseEvent('parlay_builder_refresh');
    } catch (error) {
        ...
        >
            ...
            {selectedPicks.length === 0 ? (
              ...
              <>
                <FlatList
                  ...
                  keyExtractor={item => `pick-${item.id}-${item.type}`}
                  horizontal
          ...
          <View style={styles.availableSection}>
            ...
            <FlatList
              ...
              keyExtractor={(item, index) => `game-${item.id || `index-${index}`}-${item.sport || 'unknown'}-${Date.now()}`}
              horizontal
              showsHorizontalScrollIndicator={false}
...
// Styles remain exactly the same as in your file 2
